...
import { getDatabase, ref, set, push, onValue, remove, update } from "https://www.gstatic.com/firebasejs/9.6.0/firebase-database.js";
...
async function barberLogin() {
    ...
    if (!name) {
        errorElement.textContent = 'الرجاء إدخال اسم الحلاق';
        errorElement.classList.remove('hidden');
        return;
    }
    ...
    try {
        // تسجيل الدخول كحلاق باستخدام Firebase Authentication
        ...
        // تحديث بيانات الحلاق في قاعدة البيانات
        await update(ref(database, 'barbers/' + currentUser.id), {
            name: name,
            phone: phone
        });
        ...
        if (error.code === 'auth/user-not-found') {
            try {
                ...
                errorElement.textContent = 'فشل في إنشاء الحساب: ' + createError.message;
                errorElement.classList.remove('hidden');
            }
        } else {
            errorElement.textContent = 'بيانات الدخول غير صحيحة: ' + error.message;
            errorElement.classList.remove('hidden');
        }
    }
}
...
async function logout() {
    try {
        ...
        document.getElementById('barberConfirmPassword').value = '';
    } catch (error) {
